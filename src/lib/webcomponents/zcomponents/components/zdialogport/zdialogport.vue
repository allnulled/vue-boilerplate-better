<template>
<div class="Component zdialogport">
    ...
    <table>
      <tbody>
        <tr>
          <td>
            <div>
              <zdialogcurrent ref="current_dialog" :port="this"></zdialogcurrent>
            </div>
          </td>
        </tr>
      </tbody>
  ...
  methods: {
    generate_html_1(html, title, footer) {
      try {
        ...
        html_vue += " </zwindowbody>";
        if (footer) {
          html_vue += " <zwindowfooter>";
          html_vue += "  <zwindowfooteritem>";
          html_vue += "   " + footer;
          html_vue += "  </zwindowfooteritem>";
          html_vue += " </zwindowfooter>";
        }
    ...
    async extract_fields_from_dialog(current_dialog) {
      try {
    ...
    get_template_from_dialog(dialogo_2) {
      try {
        let html_vue = undefined;
        if (dialogo_2.type === "form") {
          html_vue = this.generate_html_1(dialogo_2.html, dialogo_2.title, dialogo_2.footer);
        } else if (dialogo_2.type === "confirm") {
          html_vue = this.generate_html_2(dialogo_2.html, dialogo_2.title, dialogo_2.footer, dialogo_2.button_accept, dialogo_2.button_reject);
        } else if (dialogo_2.type === "inform") {
          html_vue = this.generate_html_3(dialogo_2.html, dialogo_2.title, dialogo_2.footer, dialogo_2.button_accept);
        } else {
    ...
    async close_first() {
      try {
        ...
        this.$vue.component("zdialogcurrent", {
          template: html_vue,
    ...
    async close_first_returning(datos) {
      try {
        const dialogo = this.dialogs.shift();
        dialogo.promise_handler.ok(datos);
        if (this.dialogs.length === 0) {
    ...
    async form(...args) {
      try {
        ...
        throw new Error("Parámetro no identificado en llamada a «zdialogport.form»");
      } catch (error) {
        console.log(error);
        throw error;
      }
    ...
    async form_by_parameters(html, title = "Mensaje", footer = false) {
      try {
    ...
    async confirm(...args) {
      try {
        ...
        throw new Error("Parámetro no identificado en llamada a «zdialogport.confirm»");
      } catch (error) {
        console.log(error);
        throw error;
      }
    ...
    async confirm_by_parameters(html, title = "Mensaje", footer = false, button_accept = "Sí", button_reject = "No") {
      try {
        const promise_handler = {};
        promise_handler.ok = undefined;
        promise_handler.fail = undefined;
        promise_handler.promise = new Promise((ok2, fail2) => {
          try {
            promise_handler.ok = ok2;
            promise_handler.fail = fail2;
          } catch (error) {
    ...
    async inform(...args) {
      try {
        ...
        throw new Error("Parámetro no identificado en llamada a «zdialogport.inform»");
      } catch (error) {
        console.log(error);
        throw error;
      }
    ...
    async error(error) {
      try {
        ...
        html = "<zlayout style='color: red;text-shadow: 0 0 2px red;'>" + html + "</zlayout>";
        ...
        this.$forceUpdate(true);
        return promise_handler.promise.then(() => {
          try {
            return error;
          } catch (error) {
            console.log(error);
